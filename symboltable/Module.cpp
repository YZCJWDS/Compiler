...
Module::Module(std::string _name) : name(_name)
{
    ...
    // 添加内置函数
    // putint: 打印一个整数
    (void) newFunction("putint", VoidType::getType(), {new FormalParam{IntegerType::getTypeInt(), "n"}}, true);
...
/// @brief 新建一个带指定类型的整型数值的Value，并加入到符号表
/// @param intVal 整数值
/// @param type 指定的类型
/// @return 常量Value
ConstInt * Module::newConstInt(int32_t intVal, Type * type)
{
    // 创建指定类型的常量
    ConstInt * val = new ConstInt(intVal, type);

    // 注意：与普通常量不同，带类型的常量不加入constIntMap
    // 因为相同整数值但不同类型的常量不能共享
...
void Module::setErrorFlag(bool flag)
{
    errorFlag = flag;
}