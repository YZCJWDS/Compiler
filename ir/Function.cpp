...
void Function::toString(std::string & str)
{
    ...
    for (auto param: params) {
        ...
        // 确保数组参数使用一致的类型表示
        std::string paramStr = "i32 " + param->getIRName();

        // 如果是数组参数，添加维度信息
        if (param->isArray()) {
            const auto & dimensions = param->getArrayDimensions();
            for (uint32_t dimension: dimensions) {
                paramStr += "[" + std::to_string(dimension) + "]";
            }
    ...
    for (auto var: varsVector) {
        ...
        // 检查是否是数组
        LocalVariable * localVar = dynamic_cast<LocalVariable *>(var);
        if (localVar && localVar->isArray()) {
            // 如果是数组，添加所有维度的数组大小
            const auto & dimensions = localVar->getArrayDimensions();
            if (!dimensions.empty()) {
                for (uint32_t dimension: dimensions) {
                    varName += "[" + std::to_string(dimension) + "]";
                }
            } else {
                varName += "[0]";
            }
    ...
    // 添加临时变量声明 (遍历指令找出所有结果值)
    std::set<std::string> declaredVars;
    for (auto & var: varsVector) {
        declaredVars.insert(var->getIRName());
    }

    for (auto & inst: code.getInsts()) {
        if (inst->hasResultValue() && declaredVars.find(inst->getIRName()) == declaredVars.end()) {
            str += "\tdeclare " + inst->getType()->toString() + " " + inst->getIRName() + "\n";
            declaredVars.insert(inst->getIRName());
        }
    }
