...
void BinaryInstruction::toString(std::string & str)
{
    ...
    switch (op) {
        ...
        case IRInstOperator::IRINST_OP_NEG_I:
            // 取反指令，一元运算 (使用"sub 0, x"格式)
            str = getIRName() + " = sub 0, " + src2->getIRName();
            break;
        ...
        case IRInstOperator::IRINST_OP_BC:
            // 条件分支指令: bc cond, label_true, label_false
            str = "bc " + src1->getIRName() + ", label " + src2->getIRName() + ", label " + getOperand(2)->getIRName();
            break;
