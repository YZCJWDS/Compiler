...
public:
    ...
    /// @brief 用于存储当前循环的上下文信息
    struct LoopContext {
        Instruction * exitLabel; // 循环退出标签
        Instruction * condLabel; // 循环条件标签，用于continue语句
    };

    /// @brief 当前嵌套循环的上下文栈
    std::vector<LoopContext> loopContextStack;

    /// @brief 进入一个循环，将其上下文信息压入栈
    /// @param exitLabel 循环退出标签
    /// @param condLabel 循环条件标签
    void enterLoop(Instruction * exitLabel, Instruction * condLabel)
    {
        loopContextStack.push_back({exitLabel, condLabel});
    }

    /// @brief 退出当前循环，弹出其上下文信息
    void exitLoop()
    {
    ...
    /// @brief 获取当前最内层循环的退出标签
    /// @return 循环退出标签，如果不在循环中则返回nullptr
    Instruction * getCurrentLoopExitLabel()
    {
        if (loopContextStack.empty()) {
            return nullptr;
        }
        return loopContextStack.back().exitLabel;
    }

    /// @brief 获取当前最内层循环的条件标签
    /// @return 循环条件标签，如果不在循环中则返回nullptr
    Instruction * getCurrentLoopCondLabel()
    {
        if (loopContextStack.empty()) {
            return nullptr;
        }
        return loopContextStack.back().condLabel;
    }
