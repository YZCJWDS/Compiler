...
ast_node * create_contain_node(ast_operator_type node_type,
                               ...
                               ast_node * fifth_child)
{
    ast_node * node = new ast_node(node_type);

    if (first_child) {
        node->insert_son_node(first_child);
    }
    if (second_child) {
        node->insert_son_node(second_child);
    }
    if (third_child) {
        node->insert_son_node(third_child);
    }
    if (fourth_child) {
        node->insert_son_node(fourth_child);
    }
    if (fifth_child) {
        node->insert_son_node(fifth_child);
    }
...
const char * getOperatorTypeName(ast_operator_type type)
{
    switch (type) {
        ...
        case ast_operator_type::AST_OP_LOGICAL_AND:
            return "AST_OP_LOGICAL_AND";
        case ast_operator_type::AST_OP_LOGICAL_OR:
            return "AST_OP_LOGICAL_OR";
        case ast_operator_type::AST_OP_CONDITIONAL:
            return "AST_OP_CONDITIONAL";
        case ast_operator_type::AST_OP_LOOP:
            return "AST_OP_LOOP";
        ...
        case ast_operator_type::AST_OP_MAX:
            return "AST_OP_MAX";
        default:
            return "Unknown AST Operator Type";
    }
}
