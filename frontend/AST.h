...
enum class ast_operator_type : int {
    ...
    AST_OP_COMPOUNDSTMT = AST_OP_BLOCK, // 保持向后兼容
...
public:
    ...
    ast_node * create_contain_node(ast_operator_type op, ast_node * child);
    ast_node * create_contain_node(ast_operator_type op, ast_node * left, ast_node * right);
...
/// @brief 创建AST的内部节点，请注意可追加孩子节点，请按次序依次加入，最多5个
